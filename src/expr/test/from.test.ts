...
describe(From, () => {
  test('reads given value.', async () => {
    ...
    await expect(new From('stuff.foo', source).run(new Flow({ onKill: jest.fn() })).execute()).resolves.toBe('bar')
  })
  ...
  test('uses the fallback if the value does not exist.', async () => {
    ...
    await expect(from.run(new Flow({ onKill: jest.fn() })).execute()).resolves.toBe('bar')
  })
  ...
  test('resolves to empty string when value not found.', async () => {
    ...
    await expect(from.run(new Flow({ onKill: jest.fn() })).execute()).resolves.toBe('')
  })
})
